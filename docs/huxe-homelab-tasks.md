...
- [ ] **台本構造**（Huxe実物に基づく）:
  ...
  6. **興味プロファイル直結セグメント**: スポーツ枠などユーザーの興味（贔屓のチーム/競技等）を直接拾う
...
- [ ] **トランジション台本**: Briefing 終了から DeepCast への橋渡し
  ...
  - DeepCast予告 + **選定理由の言語化**: 「ユーザーが好きな音楽ジャンルのルーツ掘りにちょうどいい」のように、興味プロファイルとの紐付けを明示
...
- [ ] **直近Nエピソードのサマリーを次回生成時のコンテキストとして渡す**: 「前回触れたトピックの続報」「先日ユーザーが関心を示した話題の関連ニュース」を入れられるように
...
### 2.5-A. 興味プロファイル管理
- [ ] **手動シード**: 興味トピックを構造化（例: 好きな音楽ジャンル, 配信者, 贔屓のスポーツ/チーム, 趣味の競技, k8s, ローカルLLM, Rust 等）。タグ＋自由記述＋重み付け
...
- [ ] ⚠️ **コールドスタート対策**: 最初の数日は履歴ゼロで手動シードだけが頼り。ここで刺さらないと使うのをやめ、履歴も溜まらない悪循環。初期シードを厚め・具体的に作り込む（「どういう切り口が好きか」まで記述。例: 「好きな音楽ジャンルはルーツ・系譜・カルチャー史の文脈で深掘りが好き」）
...
- [ ] **LLM 再ランキング**: 類似度上位の候補を LLM に渡し、「関連するが目新しい/見落としがちな」ものを選ぶ。"近すぎる=既知" を弾く目利き層
  - プロンプトに「既に知っていそうな定番は避け、関連はするが意外性のあるものを優先」「ユーザーが好きな切り口（ルーツ・系譜・カルチャー史等）を意識」等の指示
...
- [ ] **DeepCastテーマ選定**: 再ランキング結果から本日のDeepCastテーマを1つ選定。**選定理由の自然言語化** を出力に含める（Phase 2-C のトランジション台本で使う）
  - 例: テーマ=「あるバンドとそのジャンルの起源」 / 理由=「ユーザーが好きな音楽ジャンルのルーツ掘りに繋がる」
- [ ] **Briefingスポーツ枠選定**: 興味直結ソース（贔屓チームの公式サイト、競技団体の情報源 等）を毎日確実に拾うパス。embedding ではなく明示的なソース指定でOK
